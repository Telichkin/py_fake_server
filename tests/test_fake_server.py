...
@pytest.mark.parametrize(
    ["route", "status"], [("/201", 201), ("/204", 204), ("/200", 200)]
)
def test_add_simple_handler(server: FakeServer, method: str, route: str, status: int):
    server.on_(method, route).response(status=status)
...
@pytest.mark.parametrize(
    "methods",
    [["get", "post"], ["post", "delete"], ["delete", "patch"], ["patch", "get"]],
)
@pytest.mark.parametrize(
    ["routes", "statuses"], [[["/same_route", "/same_route"], [201, 200]]]
)
def test_add_two_simple_handlers_with_same_route(
    server: FakeServer, methods: List[str], routes: List[str], statuses: List[int]
):
    server.on_(methods[0], routes[0]).response(status=statuses[0])

    server.on_(methods[1], routes[1]).response(status=statuses[1])
...
def test_handler_returns_body(server: FakeServer):
    server.on_("get", "/try/body").response(status=200, body="Wow! So Body!")
...
def test_handler_returns_content_type(server: FakeServer):
    server.on_("post", "/content").response(
        status=200, body='{"json": "try"}', content_type="application/json"
    )
...
def test_handler_returns_headers(server: FakeServer):
    server.on_("post", "/data").response(
        status=200, body="text", headers={"retry-after": "500"}
    )
...
def test_handler_exception_when_content_type_explicit_and_in_headers(
    server: FakeServer,
):
...
def test_handler_returns_cookies(server: FakeServer):
    server.on_("get", "/users").response(
        status=200, body="lol", cookies={"token": "new_token"}
    )
...
def test_handler_exception_when_cookies_explicit_and_in_headers(server: FakeServer):
    with pytest.raises(AttributeError) as error:
        server.on_("get", "/movies/1").response(
            status=204, cookies={"token": "token"}, headers={"cookies": "token=token"}
        )
...
def test_handler_set_for_many_times(server: FakeServer):
    server.on_("get", "/users").response(status=200).then().response(status=204)
...
def test_set_handler_many_times(server: FakeServer):
    server.on_("get", "/users").response(status=204)

    server.on_("get", "/users").response(status=200)
...
def test_handler_calls_many_times(server: FakeServer):
    server.on_("patch", "/channels").response(status=204)
...
def test_handler_set_number_of_calls_explicitly(server: FakeServer):
    server.on_("get", "/games").response(status=200).once()
    ...
    assert (
        response_1.text
        == "Server has not responses for [GET] http://localhost:8081/games"
    )
    assert response_1.headers["content-type"] == "text/plain"


def test_handler_two_responses_both_called_once(server: FakeServer):
    server.on_("get", "/games").response(status=200).once().then().response(
        status=204
    ).once()
...
def test_handler_called_twice(server: FakeServer):
    server.on_("post", "/songs").response(status=204).twice()
    ...
    assert (
        response_2.text
        == "Server has not responses for [POST] http://localhost:8081/songs"
    )
    assert response_2.headers["content-type"] == "text/plain"


def test_handler_called_nth_times(server: FakeServer):
    server.on_("patch", "/songs").response(status=204)._3_times()
    ...
    assert (
        response_3.text
        == "Server has not responses for [PATCH] http://localhost:8081/songs"
    )
...
def test_handler_called_nth_times_with_wrong_syntax(server: FakeServer):
    with pytest.raises(AttributeError) as error:
        server.on_("post", "/plays").response(status=200, body="lol")._56_toims()
...
def test_handler_called_one_time_other_syntax(server: FakeServer):
    server.on_("get", "/plays").response(status=200, body="Wow!")._1_times()
    ...
    assert (
        response_1.text
        == "Server has not responses for [GET] http://localhost:8081/plays"
    )
    assert response_1.headers["content-type"] == "text/plain"
...
def test_missing_route_returns_500(server: FakeServer):
    ...
    assert (
        response.text
        == "Server has not responses for [POST] http://localhost:8081/missing_path"
    )
    assert response.headers["content-type"] == "text/plain"


def test_handler_204_without_body(server: FakeServer):
    server.on_("delete", "/user/1").response(status=204)
...
def test_json_in_response(server: FakeServer):
    (server.on_("get", "/json_response").response(status=200, json={"hello": "world"}))
...
def test_json_and_body_raise_exception(server: FakeServer):
    with pytest.raises(AttributeError) as error:
        (
            server.on_("post", "/error").response(
                status=200, body="Oops!", json={"oops": "I did it again"}
            )
        )
...
def test_explicit_content_type_remove_application_json(server: FakeServer):
    (
        server.on_("patch", "/games").response(
            status=200, json={"change": "content-type"}, content_type="text/plain"
        )
    )
...
def test_response_on_query_parameters(server: FakeServer):
    server.on_(
        "get",
        "/query",
        {"param1": "value1", "param2": "value2", "param3": ["value3", "value4"]},
    ).response(status=200, body="It works !")
    response = requests.get(
        server.base_uri
        + "/query?param1=value1&param2=value2&param3=value3&param3=value4"
    )
    assert response.status_code == 200
    assert response.text == "It works !"


def test_response_on_wrong_query_parameters(server: FakeServer):
    server.on_(
        "get",
        "/query",
        {"param1": "value2", "param2": "value2", "param3": ["value3", "value4"]},
    ).response(status=200, body="It works !")
    response = requests.get(
        server.base_uri
        + "/query?param1=value1&param2=value2&param3=value3&param3=value4"
